...
import ec.europa.digit.config.KafkaProperties;
...
class RedressKafkaResourceIT {
    ...
    void setup() {
        ...
        RedressKafkaResource kafkaResource = new RedressKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/redress-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/redress-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
